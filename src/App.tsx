...
function App() {
  ...
  */
  ...
  if (
    ...
    window.location.href.includes("listen_to_me")
  ) {
    const wrapper: any = document.querySelector(
      'div[jstcache][aria-label][role="main"][jsan][data-js-log-root]'
    );
    const data: any = {};
    data.Title = wrapper.querySelector("h1").innerText; //Example element queries from the dom on the screen.
    data.Address = wrapper.querySelector(
      "div.RcCsl.fVHpi.w4vB1d.NOE9ve.M0S7ae.AG25L button"
    ).innerText; //Example element queries from the dom on the screen.
          ...
          onClick={() => {
            ...
            */
            const places = document.querySelectorAll(
              'div > div[role="article"] > a[jsaction]' //[href*="www.google."]
            );
            ...
            places.forEach((link, index) => {
              ...
              setTimeout(() => {
                console.log(
                  "new tab reading.. left: " + (places.length - index - 1)
                );
                chrome.runtime.sendMessage({
                  create: { url: url, active: false, pinned: true },
                });
              }, 2000 * index);
            });
